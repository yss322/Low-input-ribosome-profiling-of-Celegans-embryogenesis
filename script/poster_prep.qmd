...
my_custom_theme <- function() {
  theme_cowplot() +
  theme(
   plot.subtitle = element_text(size = unit(24,'pt')),
    text = element_text(family = "Helvetica"),
     legend.title = element_text(size = unit(24,"pt")), 
    legend.text = element_text(size = unit(24,"pt")),
    axis.title = element_text(size = unit(24,"pt")),
...
plot_pairwise_relationships <- function(counts_w, 
  ...
  sp <- ggplot(counts_w, aes_string(x = id1, y = id2)) +
    geom_jitter(size = 1, color = "gray", alpha = 0.3) +
...
plot_pairwise_relationships_reliability<- function(counts_w, 
  ...
  sp <- ggplot(data_copy, aes(x = mean_id1_cpm, y = mean_id2_cpm)) +
    ...
    geom_text(data = annotation_df, aes(x = x, y = y, label = label),
              parse = TRUE, hjust = 0, vjust = 1, size = 10) +
    labs(subtitle = main,
...
scale_fill_gradientn(colors = c("white", "#e6f3f8", "#005f86"),
    ...
    theme(axis.line = element_line(size = 2),
          ...
          axis.text.y = element_text(size = 24),
          legend.text = element_text(size = 24,angle = 45, hjust = 1),
          legend.title = element_text(size = 24),
...
start_site_wt <-ggplot(wt_df_start_ex, aes(x = position_a_site, y = count, color = experiment)) + 
  ...
  scale_y_continuous(limits = c(0,1500))+ scale_x_continuous(
    ...
    expand = c(0, 0)) + 
  xlab("Position")+ylab("Counts per million")+
  scale_x_continuous(
    limits = c(-30, 30),
...
plot_TE_2v1 <- create_MD_plot(contrast_list[["qlf_TE_two_vs_one"]]) +
  ...
  my_custom_theme() + 
  theme(plot.subtitle = element_text(size = unit(24, 'pt'), hjust = 0.5))  +scale_y_continuous(
    limits = c(-10, 10),
    breaks = seq(-10,10, by = 2),
    labels = seq(-10, 10, by = 2),
    expand = c(0, 0)
...
Ribo_vs_RNA_one_to_two_plot<-ggplot(Ribo_vs_RNA_one_to_two, aes(x = RNA_logFC, y = ribo_logFC, color = logFC_TE)) + 
  ...
  theme(panel.grid.major.y = element_line(color = "grey", size = 1, linetype = 1)) + 
  ...
  geom_hline(yintercept = 0, linetype = "dotted", color = "black",size = 2) +
  ...
  my_custom_theme()+ labs(subtitle = "2-cell vs 1-cell") + theme(plot.subtitle = element_text(size = unit(24,'pt')))
...
Ribo_vs_RNA_four_to_two_plot<-ggplot(Ribo_vs_RNA_four_to_two, aes(x = RNA_logFC, y = ribo_logFC, color = logFC_TE))+ 
  ...
  geom_vline(xintercept = 0, linetype = "dotted", color = "black", size =2 ) +
   geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black", size =2 ) + 
  my_custom_theme()+labs(subtitle = "4-cell vs 2-cell") + theme(plot.subtitle = element_text(size = unit(24,'pt')))
...
localization_of_mrna <- p + 
  ylim(c(-5, 5)) +
  guides(color = guide_legend(override.aes = list(shape = NA, linetype = 1, size = 1)))+ theme(legend.position = "top",
        legend.justification = "center")
...
# what is the TE of transcripts actively in P-granule at each stage

p_granule_table <- readxl::read_xlsx("/Users/yashshukla/Desktop/p-granule_localiztion.xlsx")
...
figure_1_B <- region_counts_percent_mean_WT_plot  + my_custom_theme()+theme(legend.position = "top",legend.key.size = unit(10, "pt"),    legend.justification = "center",
    legend.box.just = "center",
    legend.text = element_text(size = unit(24,"pt"))) 

figure_1_C <- start_site_wt + stop_site_wt + 
  plot_layout(guides = 'collect', axes = "collect") &
  theme(
    legend.position = "top",
    legend.title = element_blank(),
    axis.text.y = element_text(size = unit(24, "pt")),
    axis.text.x = element_text(size = unit(20, "pt"), angle = 45, hjust = 1)
  )
...
figure_1_E<- one_cell_WT_RNA_vs_RIBO_plot+
  ...
  plot_layout(guides = "collect", ncol = 1, nrow= 4)&theme(legend.position = "none", plot.subtitle = element_text(size = unit(24,"pt")))&xlab("RNA abundance (CPM)")&ylab("Ribosome Occupancy (CPM)")
...
figure_2_D <- Ribo_vs_RNA_one_to_two_plot+Ribo_vs_RNA_four_to_two_plot + plot_layout(axes = 'collect') & theme( legend.text = element_text(size = 20), plot.subtitle = element_text(hjust = 0.5,size = 24)) 
...
Figure_S2_B <- plot_RNA_8v1 + plot_Ribo_8v1 + plot_TE_8v1 +plot_layout(axes = 'collect', guides = 'collect') 
...
ggsave(
    "./output/Figures/Figure_S2/Figure_S2_B.pdf",
    plot = Figure_S2_B,
    width =180,
    height =100,
    units = c("mm"),
...
Figure_S3_A <- example_from_publication 
...
Figure_S4_A <- gld_1_pull_down_with_stats 
Figure_S4_B <- pos_1_motif_plot_with_stats
Figure_S4_C <- mex_3_motif_plot_with_stats 
Figure_S4_D <- gld_1_motif_plot_with_stats
Figure_S4_E <- lin_41_motif_plot_with_stats 
...
Figure_S5_F <- OMA_1_pull_down_mutant_with_stats 
...
```{r}
Figure_S7_A <- p_granule_all_violin  
Figure_S7_B <- P_granule_boxplot 

Panel_S7A_S7B <- Figure_S7_A + Figure_S7_B 

ggsave(
    "./output/Figures/Figure_S7/Panel_S7A_S7B.pdf",
    plot = Panel_S7A_S7B ,
    width = 190,
    height =100,
    units = c("mm"),
