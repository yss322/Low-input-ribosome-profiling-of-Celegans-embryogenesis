...
create_MD_plot <- function(contrast, genes_to_label = c()) {
  ...
  plot_MD <- ggplot(output, aes(x = logCPM, y = logFC)) +
    ...
    scale_fill_gradient(low = "white", high = "#005f86", name = "Density", 
                        limits = c(0, 0.35)) +
...
background_data <- read.csv("/Users/yashshukla/Documents/Computational/output/pairwise_translation_analysis/all_detected.csv")
...
enrichment_df <- enrichment_df |> filter(enrichment_ratio> 2)

write_csv(enrichment_df, "/Users/yashshukla/Documents/Computational/Celegans_riboITP/paper_draft/20241022_Paper_draft/enrichment_df.csv")

enrichment_df_mod <- read_csv("/Users/yashshukla/Downloads/enrichment_df_modified.csv")
...
# Prepare data (CLR-transformed data, no scaling needed)
prepare_data <- function(data) {
  gene_names <- data$gene_name
...
# Count genes and create Roman numeral labels
gene_counts <- combined_results %>%
  count(combined_cluster) %>%
  mutate(
    cluster_num = row_number(),
    roman_numeral = sapply(cluster_num, to_roman),
    label = paste0("Cluster ", roman_numeral, "\n(", n, " genes)")
  )

# Create the main plot
multimodal_plot <- ggplot(cluster_profiles, 
  ...
  facet_wrap(~ combined_cluster, nrow = 1,
             labeller = labeller(combined_cluster = setNames(gene_counts$label, 
                                                             gene_counts$combined_cluster))) +
  coord_cartesian(ylim = c(-2.5, 2.5)) +
  labs(
    x = "Cell-stage",
    y = "TE value (relative to 1-cell stage)",
...
gene_lists <- plot_data %>%
  distinct(gene_name, combined_cluster) %>%
...
# Clean results
cleaned_results <- simplify(enrichment_results)

# Process enrichment data
enrichment_df <- as.data.frame(cleaned_results) %>%
  mutate(
    GeneRatio_num = as.numeric(sapply(strsplit(GeneRatio, "/"), "[", 1)),
    GeneRatio_denom = as.numeric(sapply(strsplit(GeneRatio, "/"), "[", 2)),
    BgRatio_num = as.numeric(sapply(strsplit(BgRatio, "/"), "[", 1)),
...
# [Previous code remains the same until the enrichment plot section]

# Modify the cluster labels to use Roman numerals that match multimodal plot
cluster_roman_map <- setNames(
  gene_counts$roman_numeral,
  gene_counts$combined_cluster
)

# Update filtered_df to use Roman numerals
filtered_df <- filtered_df %>%
  mutate(
    # Convert Cluster to Roman numerals using the mapping
    Cluster_Roman = cluster_roman_map[Cluster],
    # Ensure the order matches the multimodal plot
    Cluster_Roman = factor(Cluster_Roman, levels = gene_counts$roman_numeral)
  )

# Create enrichment plot with Roman numerals
enrichment_plot_oma <- ggplot(filtered_df,
                          aes(x = Cluster_Roman,
                              y = Description)) +
  geom_point(aes(size = enrichment_ratio,
                 color = -log10(p.adjust))) +
...
Figure_7_B <- enrichment_plot_oma 



ggsave( "./output/Figures/Figure_7/Figure_7_A.pdf",multimodal_plot, width = 180, height = 75, units = "mm", dpi = 600, limitsize = F)
ggsave( "/Users/yashshukla/Documents/Computational/Celegans_riboITP/paper_draft/20241022_Paper_draft/Figures/Figure_7/Figure_7_B.pdf",Figure_7_B , width = 180, height = 150, units = "mm", dpi = 600, limitsize = F)
```
